...
1. Use the `view` tool to open the integration-summary path the user gave you, end-to-end. Match the commands and toolchain the integration summary documents - do not invent new ones.

2. The integration-summary filename carries your run-name. Strip the directory and the `-integration-summary.md` suffix. You'll use it to name your output file.

3. If no path was given, ask the user for the integration summary path. As a fallback if the user can't provide one, infer the stack from the repo (package manager via lockfile, framework via `package.json`).
